  ...
  return (
    <header  className={`z-40 ${themeConfig.semidark && themeConfig.menu === "horizontal" ? "dark" : ""}`}>
      <div  className="shadow-sm">
        <div style={{ justifyContent: "end" }}   className="relative bg-white flex w-full items-center px-5 py-2.5 dark:bg-black">
          <div className="horizontal-logo flex lg:hidden justify-between items-center">
            <Link to="/" className="main-logo flex items-center shrink-0">
              <img className="w-8 inline" src="/assets/images/logo.svg" alt="logo" />
            ...
            <button
              type="button"
              className="collapse-icon flex-none dark:text-[#d0d2d6] hover:text-primary p-2 rounded-full bg-white-light/40 dark:bg-dark/40"
          ...
          <div className="flex items-center space-x-2 rtl:space-x-reverse dark:text-[#d0d2d6]">
            <div>
              <button
                type="button"
                onClick={() => setSearch(!search)}
                className="search_btn p-2 rounded-full bg-white-light/40 dark:bg-dark/40"
              >
                <Search size="20" />
              </button>
            </div>
                ...
                <ul className="py-0 text-dark dark:text-white-dark w-[300px] divide-y dark:divide-white/10">
                  ...
                  {notifications.length > 0 ? (
                    ...
                    <li>
                      <button
                        type="button"
                        className="grid place-content-center text-lg min-h-[200px]"
                      >
                  ...
                  <li>
                    <div className="flex items-center px-4 py-4">
                      ...
                      <div className="pl-4 truncate">
                        ...
                        <button
                          type="button"
                          className="text-black/60 hover:text-primary dark:text-dark-light/60"
                        >
                          {user?.email}
                        </button>
                      </div>
...
//     console.log("New Notification:", message); // Log notification
//     const newNotification = {
//       id: Date.now(), // Unique ID for each notification
//       message,
//       time: new Date().toLocaleTimeString(), // Timestamp
//       profile: "user-profile.jpeg", // Replace with actual profile image if available
//     };
//     setNotifications((prev) => [newNotification, ...prev]); // Add to the top of the list
//   };

//   // WebSocket connection to listen for new orders and updates
//   useEffect(() => {
//     const socket = io("http://localhost:3005/socket"); // Replace with your WebSocket server URL

//     socket.on("connect", () => {
//       console.log("WebSocket connection established");
//       socket.emit("joinRoom", id); // Join the room for this user
//     });
...
//       console.log("Received newCommand:", data); // Debugging
//       if (data.delivererId === id || data.clientId === id || data.restoManager === id) {
//         addNotification(`New order: ${data.orderId} (Status: ${data.status})`);
//       }
//     });

//     // Listen for order updates
//     socket.on("orderUpdate", (data) => {
//       console.log("Received orderUpdate:", data); // Debugging
//       if (data.delivererId === id || data.clientId === id || data.restoManager === id) {
//         addNotification(`Order updated: ${data.orderId} (Status: ${data.status})`);
//       }
//     });

//     // Listen for delivered orders
//     socket.on("orderDelivred", (data) => {
//       console.log("Received orderDelivred:", data); // Debugging
//       if (data.restoManager === id) {
//         // Check if order status is delivered
//         if (data.status === "delivered") {
//           addNotification(`Order ${data.orderId} has been delivered successfully`);
//         }
//       }
//     });
...
//                     placeholder="Search jldjd..."
//                   />
//                   <button
//                     type="button"
//                     className="absolute w-9 px-2 h-9 inset-0 ltr:right-auto rtl:left-auto appearance-none peer-focus:text-primary"
//                   >
